...
- `run-tests.sh` exists and is executable — read it directly from disk at `run-tests.sh` (repo root) using your file-reading tool; do not rely on it appearing in the provided implementation files, as it lives outside the frontend/backend directories. A missing or unreadable script is a blocking failure.
...
- UX spec behavioral requirements without a Gherkin scenario are **Gherkin coverage gaps, not implementation gaps**. The developer spec defines Gherkin as authoritative for behavior and explicitly prohibits implementing behavior not backed by a Gherkin scenario. If the UX spec describes behavior (e.g. keyboard navigation, focus management, animation) that has no corresponding Gherkin scenario, do not block — note it under "Gherkin coverage gaps" in your output summary for the calibrator to address. Only block when a Gherkin scenario's behavior is missing or incorrect in the implementation.
