...
function MetricTile({ label, value, unit, testId, ariaLabel }: MetricTileProps) {
  return (
    ...
    >
      <span
        style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)', textTransform: 'uppercase', letterSpacing: '0.06em' }}
      >
        {label}
      </span>
      <span
      ...
      >
        {value}
        {unit && (
          <span style={{ fontSize: 12, fontWeight: 400, marginLeft: 4, color: 'rgba(255,255,255,0.6)' }}>
            {unit}
          </span>
        )}
      </span>
...
function TrendChip({ direction, testId, ariaLabel }: TrendChipProps) {
  return (
    <span data-testid={testId} aria-label={ariaLabel} style={trendChipStyle(direction)}>
...
function WeeklySummaryCard({ week, previousWeek }: WeeklySummaryCardProps) {
  ...
  const prevAvgCadence = previousWeek ? computeWeeklyAvgCadence(previousWeek.activities) : 0
  ...
  return (
    ...
    >
      <h2 style={{ margin: '0 0 12px 0', fontSize: 16, color: 'rgba(255,255,255,0.7)', fontWeight: 600 }}>
        Weekly Summary
      </h2>
      <div
        style={{
      ...
      <div style={{ marginTop: 12 }}>
        <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
          Intensity Balance
        </span>
      ...
      <div style={{ marginTop: 12 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 6 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)', minWidth: 100 }}>Training Load</span>
            <TrendChip
              direction={trendLoad}
              testId="trend-training-load"
              ariaLabel={trendLoad === 'none' ? 'Training load trend: no comparison available' : `Training load trend: ${trendLabel(trendLoad)}`}
            />
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)', minWidth: 100 }}>Avg HR</span>
            <TrendChip
              direction={trendAvgHr}
              testId="trend-avg-hr"
              ariaLabel={trendAvgHr === 'none' ? 'Average HR trend: no comparison available' : `Average HR trend: ${trendLabel(trendAvgHr)}`}
            />
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)', minWidth: 100 }}>Resting HR</span>
            <TrendChip
              direction={trendRestingHr}
              testId="trend-resting-hr"
              ariaLabel={trendRestingHr === 'none' ? 'Resting HR trend: no comparison available' : `Resting HR trend: ${trendLabel(trendRestingHr)}`}
            />
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      {activities.map((activity) => (
        <li key={activity.id} role="listitem" style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          <button
            type="button"
            ...
            style={{
              ...
              gap: 12,
              width: '100%',
              background: selectedId === activity.id ? 'rgba(255,255,255,0.1)' : 'transparent',
              border: 'none',
              color: 'inherit',
              padding: '12px 16px',
              cursor: 'pointer',
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>{activity.name}</h2>
        <button
          type="button"
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: 14 }}>Type</span>
          <span style={{ fontSize: 14 }}>{activity.type}</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: 14 }}>Duration</span>
          <span style={{ fontSize: 14 }}>{activity.durationMin}m</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: 14 }}>Distance</span>
          <span style={{ fontSize: 14 }}>{activity.distanceKm.toFixed(1)} km</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: 14 }}>Avg HR</span>
          <span
            data-testid="activity-avg-hr"
            aria-label={hasAvgHr ? `Average heart rate: ${activity.avgHr} bpm` : 'Average heart rate: not available'}
            style={{ fontSize: 14 }}
          >
            {hasAvgHr ? (
              <>
                {activity.avgHr}
                <span style={{ fontSize: 12, marginLeft: 4, color: 'rgba(255,255,255,0.6)' }}>bpm</span>
              </>
            ) : (
              EM_DASH
            )}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: 14 }}>Cadence</span>
          <span
            data-testid="activity-cadence"
            aria-label={hasCadence ? `Cadence: ${activity.cadence} spm` : 'Cadence: not available'}
            style={{ fontSize: 14 }}
          >
            {hasCadence ? (
              <>
                {activity.cadence}
                <span style={{ fontSize: 12, marginLeft: 4, color: 'rgba(255,255,255,0.6)' }}>spm</span>
              </>
            ) : (
              EM_DASH
            )}
...
export function WeeklyDashboard({ extraActivities, overrideDataset }: WeeklyDashboardProps) {
  const baseDataset = overrideDataset ?? require('../data/weeklyDashboardData').weeklyDashboardDataset as WeekData[]
  
  // Sort descending to default to most recent
  const sorted = [...baseDataset].sort((a, b) => b.weekId.localeCompare(a.weekId))
  const [selectedWeekId, setSelectedWeekId] = useState<string>(sorted[0]?.weekId ?? '')
  ...
  return (
    ...
    >
      <h1 style={{ fontSize: 24, margin: '0 0 16px 0' }}>Weekly Training Dashboard</h1>
      ...
      >
        {currentWeek && (
          <WeeklySummaryCard week={currentWeek} previousWeek={previousWeek} />
        )}
      ...
      {selectedActivity && (
        <div style={{ marginTop: 16 }}>
          <ActivityDetail activity={selectedActivity} onClose={handleDetailClose} />
        </div>
      )}
    </div>
  )
